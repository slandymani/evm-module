...
type Config struct {
	...
	GenesisState      evmmoduletypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("evmm_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
		BondDenom:         "aISLM",
		MinGasPrices:      fmt.Sprintf("0.000006%s", evmmoduletypes.AttoDenom),
...
// NewAppConstructor returns a new EvmM AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "evmmd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "evmmcli")
		...
		customAppTemplate, _ := config.AppConfig(evmmoduletypes.AttoDenom)
