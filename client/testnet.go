...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(evmmoduletypes.AttoDenom, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(evmmoduletypes.AttoDenom, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(evmmoduletypes.AttoDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, evmmoduletypes.AttoDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
