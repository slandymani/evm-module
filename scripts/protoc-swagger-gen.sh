...
ln -snf ../../proto/evmm "${SWAGGER_PROTO_DIR}/proto/evmm"
...
cat tmp-swagger-gen/apidocs.swagger.json | jq '.info.title |= "Evmm gRPC Gateway API"' | jq '.info.version |= "0.1.0"' > client/docs/swagger-ui/swagger.json
