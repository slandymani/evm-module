...
## default port prefixes for evmmd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    ...
    "$PWD"/build/evmmd keys add $KEY_NAME --keyring-backend test --home $HOME_DIR --no-backup --algo "eth_secp256k1"
    "$PWD"/build/evmmd init $MONIKER --chain-id $CHAINID --home $HOME_DIR
    ...
    "$PWD"/build/evmmd add-genesis-account \
    "$("$PWD"/build/evmmd keys show "$KEY$i" --keyring-backend test -a --home "$HOME_DIR")" 1000000000000000000aISLM \
    --keyring-backend test --home $HOME_DIR
    "$PWD"/build/evmmd gentx "$KEY$i" 1000000000000000000aISLM --chain-id $CHAINID --keyring-backend test --home $HOME_DIR
    "$PWD"/build/evmmd collect-gentxs --home $HOME_DIR
    "$PWD"/build/evmmd validate-genesis --home $HOME_DIR
...
start_func() {
    ...
    "$PWD"/build/evmmd start --pruning=nothing --rpc.unsafe \
    ...
    EVMM_PID=$!
    echo "started evmm node, pid=$EVMM_PID"
    # add PID to array
    arr+=("$EVMM_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        HOME_DIR="$DATA_DIR$i"
        PRIV_KEY=$(./build/evmmd keys unsafe-export-eth-key $KEY_NAME --home $HOME_DIR --keyring-backend test)
        echo $PRIV_KEY

        sleep 5

        HOST_RPC=http://$IP_ADDR:$RPC_PORT"$i"
        echo "going to test evmm node $HOST_RPC ..."
...
stop_func() {
    EVMM_PID=$i
    echo "shutting down node, pid=$EVMM_PID ..."
