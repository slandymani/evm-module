...
import (
	...
	cmdcfg "github.com/slandymani/evm-module/cmd/config"
	"github.com/slandymani/evm-module/crypto/hd"
	evmmkr "github.com/slandymani/evm-module/crypto/keyring"
	"github.com/slandymani/evm-module/server/config"
	srvflags "github.com/slandymani/evm-module/server/flags"
	"github.com/slandymani/evm-module/testutil/network"
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", cmdcfg.AttoDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
Example:
	evmmd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "evmmd", "Home directory of the node's daemon configuration")
...
Example:
	evmmd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("evmm_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, evmmkr.Option())
		if err != nil {
			return err
		}
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, cmdcfg.AttoDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
