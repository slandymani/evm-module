...
SIMAPP = ./app
HTTPS_GIT := https://github.com/slandymani/evm-module.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=evmm \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(EVMM_BINARY) \
...
build-docker:
	...
	$(DOCKER) create --name evmm -t -i ${DOCKER_IMAGE}:latest evmm
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp evmm:/usr/bin/evmmd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/slandymani/evm-module/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(EVMM_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(EVMM_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(EVMM_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(EVMM_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format: 
	...
	find . -name '*.go' -type f -not -path "./.devenv/*" -not -path "./vendor*" -not -path "*.git*" -not -path "./.direnv/*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/slandymani/evm-module
...
PACKAGE_NAME:=github.com/slandymani/evm-module
